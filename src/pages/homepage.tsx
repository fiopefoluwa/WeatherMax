...
export const Homepage = () => {
  ...
  // Extract coordinates and return them
  const extractCoordinates = (location: string): { lat: number | null; lng: number | null } => {
    try {
      const city = cities.find(
        (city: City) => city.name.toLowerCase() === location.toLowerCase()
      );
      if (city) {
  ...
  const makeWeatherRequest = async (lat: number, lng: number) => {
    ...
    try {
      const res = await axios.get(`${constants.API}&latitude=${lat}&longitude=${lng}`);
    ...
    <>
      <div className="max-w-lg w-screen mx-auto py-12 px-4">
        ...
        {!(isLoaded || isFetching) && (
          <div className="py-4">
            <div className="justify-center flex select-none">
              <img src={GreyCloudIcon} alt="Cloud Image" width={72} />
            </div>
            <div className="text-center text-[#7E839A] font-normal transform translate-y-2">
              <p>Type in your city to get</p>
              <span>weather forecasts.</span>
        ...
        {isError ? (
          ...
          isLoaded && weatherData && (
            <WeatherStats location={searchCity} weatherData={weatherData} />
          )
        )}
      </div>
    </>
